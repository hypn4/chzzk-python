...
def watch(
    ...
    async def run_chat() -> None:
        async with AsyncUnofficialChzzkClient(nid_aut=nid_aut, nid_ses=nid_ses) as client:
            ...
            # Check if live (unless --offline is specified)
            if not live_detail.is_live and not offline:
                if json_output:
            ...
            try:
                ...
                if json_output:
                    console.print(json.dumps({"error": "Channel is not live"}))
                else:
                    console.print("[red]Error:[/red] Channel is not live")
                raise typer.Exit(1) from None
...
def send(
    ...
    message: Annotated[
        str,
        typer.Argument(help="Message to send"),
    ],
) -> None:
